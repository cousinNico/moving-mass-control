...
private:
    ...
    double speedInt_to_pps(int32_t v_int, int pulse_div){      // eq. (1) in manual
        // v_pps = 16e6 * v_int / ( 2^pulse_div * 2048 * 32 )
        return (16e6 * double(v_int)) / ( (1u << pulse_div) * 2048.0 * 32.0 );
    }

    double pps_to_rpm(double v_pps, int microstep_res_pow2, int fullsteps_per_rev){
        // AP140 stores microstep resolution as a power-of-two index (0=full,1=half,...)
        double fullstep_hz = v_pps / double(1u << microstep_res_pow2);
        return 60.0 * fullstep_hz / double(fullsteps_per_rev);
    }
